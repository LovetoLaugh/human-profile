...
export class ProfileService {
 constructor(private store: RepositoryStore, private now: () => string = () => new Date().toISOString(), private initialization: 'demo' | 'empty' = 'demo') {}
 private async initialize(ownerId: string, repositories: Repositories) {
  ...
  if (this.initialization === 'demo' && !commitments.length && !evidence.length) {
  ...
  const defaults = this.initialization === 'empty' ? emptyProfile() : { about: initialAbout, permissions: initialPermissions };
  await repositories.profile.save({ ...defaults, ownerId });
 }
